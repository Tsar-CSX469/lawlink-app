...
class ActListPageState extends State<ActListPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ...
          : acts.isEmpty
              ? const Center(child: Text('No acts found. Check if PDF is correctly loaded.'))
              : ListView.builder(
  ...
  Widget _buildStructuredContent(String content) {
    ...
    if (content.contains('WHEREAS')) {
      ...
      final preambleEnd = content.indexOf('NOW THEREFORE', preambleStart);
      if (preambleEnd > preambleStart) {
        sections.add(const Text('PREAMBLE', style: TextStyle(fontWeight: FontWeight.bold)));
        sections.add(const SizedBox(height: 8));
        sections.add(Text(content.substring(preambleStart, preambleEnd).trim()));
        sections.add(const SizedBox(height: 16));
      }
    }
    ...
    if (parts.isNotEmpty) {
      for (final part in parts) {
        ...
        try {
          final nextPart = parts.firstWhere(
            (m) => m.start > partStart,
          );
          ...
          child: Column(
            ...
            children: [
              ...
              Expanded(
                child: SingleChildScrollView(
                  padding: const EdgeInsets.all(16),
                  child: SelectableText(rawPdfText.isEmpty 
