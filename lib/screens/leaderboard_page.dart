...
class _LeaderboardPageState extends State<LeaderboardPage>
  ...
  final List<QuizCategory> _quizCategories = [
    ...
    QuizCategory(id: 'consumer_affairs_quiz', name: 'Consumer Law', icon: Icons.shopping_cart),
    QuizCategory(id: 'criminal_law_quiz', name: 'Criminal Law', icon: Icons.gavel),
    QuizCategory(id: 'civil_law_quiz', name: 'Civil Law', icon: Icons.balance),
    QuizCategory(id: 'employment_law_quiz', name: 'Employment Law', icon: Icons.work),
    QuizCategory(id: 'family_law_quiz', name: 'Family Law', icon: Icons.family_restroom),
  ];
  ...
  List<LeaderboardEntry> getSortedEntries({String? quizFilter}) {
    ...
    for (LeaderboardEntry entry in entries) {
      String key = quizFilter == 'all' 
          ? '${entry.userId}_${entry.quizId}' 
          : entry.userId;
      
      if (!bestScores.containsKey(key) || 
          (entry.score / entry.total) > (bestScores[key]!.score / bestScores[key]!.total)) {
      ...
      body: Container(
        ...
        child: _isLoading
            ...
            : _error != null
                ...
                : TabBarView(
                    controller: _tabController,
                    children: _quizCategories.map((category) => 
  ...
  Widget _buildEmptyState(String? quizFilter) {
    String categoryName = _quizCategories
        .firstWhere((cat) => cat.id == quizFilter, 
                   orElse: () => QuizCategory(id: 'all', name: 'Overall', icon: Icons.leaderboard))
        .name;
